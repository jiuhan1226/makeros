...
export default function BookmarkPage({ wrongNotes, history, certificateName, onStartRecommended, onStartWrongReview, repeatedWrong = [], dueReviews = [], onStartDueReview }) {
  ...
  return <main className="page-shell cbt-only-page">
    ...
    <section className="cbt-wrong-overview cbt-wrong-overview-v2">
      <article className="panel"><strong>{wrongNotes.length}</strong><span>복습할 오답</span></article>
      <article className="panel"><strong>{repeatedWrong.length}</strong><span>2회 이상 반복 오답</span><button className="text-button" disabled={!repeatedWrong.length} onClick={() => onStartWrongReview?.(repeatedWrong)}>집중복습</button></article>
      <article className="panel"><strong>{dueReviews.length}</strong><span>오늘 자동 복습</span><button className="text-button" disabled={!dueReviews.length} onClick={() => onStartDueReview?.(dueReviews)}>복습 시작</button></article>
    ...
    <section className="review-list">{filtered.map((item, index) => {
      ...
      return <article className="wrong-note-card detailed-cbt-wrong" key={key}>
        <div className="wrong-card-top"><label className="wrong-select"><input type="checkbox" checked={selected.includes(key)} onChange={() => toggle(key)}/> 선택</label><div><span className="content-source-chip cbt-source-chip">CBT</span><strong>{item.subject || "공통"}</strong><small>{Number(item.wrongCount || 1)}회 틀림</small></div><button className="secondary" onClick={() => onStartWrongReview?.([item])}>다시 풀기</button></div>
        <h3>{item.question}</h3>
        <p>내 답: {item.selectedAnswerIndex == null ? "미응답" : circled[item.selectedAnswerIndex]} · 정답: {circled[item.answerIndex]}</p>
        <details><summary>문제 자세히 보기</summary>{(item.questionImageUrls?.length ? item.questionImageUrls : (item.imageUrl ? [item.imageUrl] : [])).map((url, imageIndex) => <img className="question-image" src={url} alt="문제 자료" key={url + imageIndex}/>)}<ol className="wrong-choice-detail">{(item.choices || []).map((choice, choiceIndex) => <li className={choiceIndex === item.answerIndex ? "correct-line" : ""} key={choiceIndex}>{circled[choiceIndex]} {choice}</li>)}</ol><div className="wrong-explanation"><strong>해설</strong><p>{item.explanation || "해설이 아직 준비되지 않았어요."}</p></div></details>
      </article>;
    })}{!filtered.length && <div className="empty-state">이 과목에는 아직 저장된 오답이 없어요.</div>}</section>
  </main>;
}
