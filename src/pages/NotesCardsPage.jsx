...
export default function NotesCardsPage({ assets, onDelete, onUpdate, onDeleteFolder, onGenerateFromWrong, busy, initialFocus }) {
  ...
  const folders = useMemo(() => buildStudyAssetGroups(rootItems), [rootItems]);
  const activeFolder = useMemo(() => folders.find((item) => item.key === folder), [folder, folders]);
  const folderItems = useMemo(() => rootItems.filter((item) => folder === "전체" || sourceGroup(item).key === folder), [rootItems, folder]);
  const items = useMemo(() => tab === "cards" ? (folder === "전체" ? [] : filterCardsByMastery(folderItems, masteryFilter)) : folderItems, [folder, folderItems, masteryFilter, tab]);
  ...
  useEffect(() => {
    ...
    setRoot(group.root); setFolder(group.key); setMasteryFilter("all"); setFocusedId(item.id);
  ...
  function removeFolder() {
    if (!activeFolder || !window.confirm(`‘${activeFolder.label}’에서 만든 AI 노트와 개념카드를 모두 삭제할까요?`)) return;
    onDeleteFolder?.(root, activeFolder.key, activeFolder.label);
    setFolder("전체");
  }
  ...
  function resetFilters(nextTab) {
    setTab(nextTab); setRoot(nextTab === "cards" ? "PDF" : "전체"); setFolder("전체"); setMasteryFilter("all");
  }
  ...
  return <main className="page-shell">
    ...
    <section className="asset-folder-layout">
      ...
      <div>
        ...
        {tab === "cards" && root !== "전체" && folder === "전체" && <section className="card-deck-overview"><header><span>{root === "PDF" ? "PDF별 개념카드" : "자격증별 개념카드"}</span><strong>학습할 자료를 먼저 선택하세요.</strong><small>선택한 자료에서 만든 카드만 한 묶음으로 학습합니다.</small></header><div>{folders.map((group) => { const deck = rootItems.filter((item) => sourceGroup(item).key === group.key); const known = deck.filter((item) => cardMastery(item) === "known").length; return <button className="panel card-deck-tile" key={group.key} onClick={() => { setFolder(group.key); setMasteryFilter("all"); }}><span>📚</span><strong>{group.label}</strong><small>{deck.length}장 · 암기 완료 {known}장</small><i><b style={{ width: `${deck.length ? (known / deck.length) * 100 : 0}%` }}/></i><em>이 카드 묶음 열기 →</em></button>; })}{!folders.length && <div className="empty-state">이 자료에서 만든 개념카드가 없습니다.</div>}</div></section>}
        {root === "PDF" && folder !== "전체" && <div className="asset-folder-toolbar"><strong>{activeFolder?.label || "PDF 학습자료"}</strong><button className="danger-text" onClick={removeFolder}>이 PDF 학습자료 전체 삭제</button></div>}
        {tab === "cards" && folder !== "전체" && <section className="classcard-toolbar panel">
          <div><span>현재 카드 묶음</span><strong>{activeFolder?.label || `${root} 카드`}</strong><small>새 카드 {masteryCounts.new} · 학습 중 {masteryCounts.learning} · 암기 완료 {masteryCounts.known}</small></div>
    ...
    {session && <div className="modal-backdrop classcard-backdrop"><section className="modal classcard-session" role="dialog" aria-modal="true" aria-labelledby="classcard-session-title"><header><div><span>{session.mode === "study" ? "카드 학습" : "선택 퀴즈"}</span><strong id="classcard-session-title">{session.finished ? "학습 완료" : `${session.index + 1} / ${session.cards.length}`}</strong></div><button onClick={() => setSession(null)} aria-label="학습 종료">×</button></header><div className="classcard-progress"><i style={{ width: `${session.finished ? 100 : ((session.index + 1) / session.cards.length) * 100}%` }}/></div>
      {session.finished ? <div className="classcard-finish"><span>오늘의 카드 학습</span><h2>{session.cards.length}장 완료</h2><div><article><strong>{session.correct}</strong><small>{session.mode === "study" ? "알아요" : "정답"}</small></article><article><strong>{session.wrong}</strong><small>{session.mode === "study" ? "다시 보기" : "오답"}</small></article></div><button className="primary" onClick={() => setSession(null)}>카드 목록으로</button></div> : session.mode === "study" ? <>
        <button className={`classcard-stage ${session.flipped ? "flipped" : ""}`} onClick={() => setSession((value) => ({ ...value, flipped: !value.flipped }))}><span>{session.flipped ? "개념 설명" : "핵심 질문"}</span><strong>{session.flipped ? currentCard.back : currentCard.front}</strong><small>{sourcePageLabel(currentCard) ? `근거 · ${sourcePageLabel(currentCard)} · ` : ""}{session.flipped ? "아래에서 기억 상태를 선택하세요" : "눌러서 답 확인 · Space"}</small></button>
