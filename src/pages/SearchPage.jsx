...
export default function SearchPage({ exams = [], searchQuestions, onOpenResult }) {
  ...
  return <main className="page-shell"><section className="page-title"><div><span className="eyebrow">SMART SEARCH</span><h1>기출문제 검색</h1><p>문제와 보기, 해설에서 원하는 내용을 찾습니다.</p></div></section>
    ...
    <section className="search-results">{results.map((q,i)=>{const exam=examMap.get(q.examId);return <button className="search-result-card" key={`${q.id}-${i}`} onClick={()=>onOpenResult(exam,q)}><span>{exam?.year || q.year}년 {exam?.round || q.round} · {q.subject}</span><strong>{q.questionNumber}. {q.question}</strong><small>{q.explanation ? `해설: ${q.explanation.slice(0,100)}` : "해설 미등록"}</small></button>})}{searched&&!results.length&&<div className="empty-state">검색 결과가 없습니다.</div>}</section>
  </main>;
}
