export default function ModeSelectPage({ exam, onStart, onBack }) {
  ...
  return <main className="page-shell"><button className="text-button" onClick={onBack}>← 기출문제</button><div className="page-heading"><h1>{exam?.title || `${exam?.year}년 ${exam?.round}`}</h1><p>학습 방식을 선택하세요.</p></div><section className="mode-grid">{modes.map(([mode, detail]) => <article className="mode-card" key={mode}><h2>{mode}</h2><p>{detail}</p><button className="primary" onClick={() => onStart(mode)}>{mode} 시작</button></article>)}</section></main>;
}
