...
export default function OpportunitiesPage({ portfolioItems = [], onChangePortfolioItems, savedIds = [], onChangeSavedIds, onAddGoal }) {
  ...
  return <main className="maker-page opportunities-page">
    ...
    {busy ? <div className="maker-card maker-inline-empty"><h3>공개 공고를 모으는 중…</h3></div> : <section className="opportunity-grid">
      {visible.map((item) => { const bookmarked = saved.includes(item.id); return <article className="maker-card opportunity-card" key={item.id}><header><span>{fieldOf(`${item.title} ${item.categories}`)}{item.sourceKind === "official" ? " · 공공기관 원문" : " · 공식 링크 확인"}</span><b>{item.dday}</b></header><h2>{item.title}</h2><p>{item.organization || item.source || "주최기관 원문 확인"}</p><div className="opportunity-audience"><span>확인된 참가 대상</span><strong>{item.audienceEvidence}</strong></div><div className="opportunity-verification">{(item.verification || []).map((entry) => <span key={entry}>✓ {entry}</span>)}</div><small>{item.source}에서 수집 · {item.deadline === "상시" ? "상시 접수" : `${item.deadline} 마감`}</small><footer><button className={bookmarked ? "saved" : ""} onClick={() => onChangeSavedIds?.(bookmarked ? saved.filter((savedId) => savedId !== item.id) : [...saved, item.id])}>{bookmarked ? "★ 저장됨" : "☆ 관심 저장"}</button><button onClick={() => addGoal(item)}>목표에 추가</button><button onClick={() => addActivity(item)}>이력서에 저장</button><a href={item.url} target="_blank" rel="noreferrer">{item.sourceKind === "official" ? "공공기관 원문" : "주최기관 원문"}</a>{item.listingUrl && <a className="secondary-link" href={item.listingUrl} target="_blank" rel="noreferrer">수집 페이지</a>}</footer></article>; })}
