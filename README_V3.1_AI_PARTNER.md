...
- **오늘**: 우선순위가 높은 행동 2개를 먼저 표시하고, 나머지 행동은 접어서 확인
- **계획**: 입력한 시작일·목표일까지의 주간 분량, 계획 버전, diff, 확정/보류/롤백
...
2. `src/utils/aiPartner.js`가 하드 제약을 먼저 적용해 입력한 목표 기간만큼 규칙 기반 계획을 만든다.
...
v3.1 파트너 상태는 로컬에서 `makeros-ai-partner-v3.1` 키로 별도 보존되며, 로그인 사용자는 Firestore 계정 상태에도 동기화됩니다. v3.1.24부터 이력서·진로·발명·프로젝트·공고 북마크와 PDF 문서·페이지·AI 노트·개념카드도 계정에 나눠 저장합니다.

상단 저장 상태는 다음 의미입니다.

- `이 기기에 저장`: 비로그인 상태로 현재 브라우저에만 저장
- `계정 불러오는 중` / `저장 중`: 계정 데이터 동기화 진행 중
- `계정에 저장됨`: 최신 변경을 계정에 저장 완료
- `동기화 확인 필요`: 네트워크 또는 저장 오류로 재확인이 필요한 상태
