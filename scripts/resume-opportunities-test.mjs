...
assert.ok(opportunities.includes("참여 기록하기") && opportunities.includes("실제로 참여했나요") && opportunities.includes("관심 저장"), "참여하지 않은 대회를 이력서로 옮기면 안 됩니다.");
assert.ok(opportunities.includes("공고에 표시된 참가 대상") && opportunities.includes("목표에 추가"), "참가 대상 표시 또는 계획 연결 누락");
